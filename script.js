...
function repaint(){
  ...
  var _lp=$("#loginToPay"), _pb=$("#payButtons"), _rb=$("#recordBtn");
  if(_lp) _lp.style.display = user ? "none" : "inline-flex";
  if(_pb) _pb.style.display = user ? "flex" : "none";
  if(_rb) _rb.style.display = user ? "inline-flex" : "none";
...
// "I've paid" -> open the record dialog (login required)
$("#recordBtn").onclick = ()=> wantPay("");
$("#inAmt").addEventListener("input", updatePayBtn);
document.querySelectorAll(".chip").forEach(c=>c.onclick=()=>{
  document.querySelectorAll(".chip").forEach(x=>x.classList.remove("on"));
  c.classList.add("on"); $("#inAmt").value=c.dataset.amt; updatePayBtn();
});
function updatePayBtn(){ $("#doPay").textContent = "Add " + money(Number($("#inAmt").value)||0) + " to my total"; }
function saveContribution(amt, paymentId){
  if(LIVE){
    fb.setDoc(fb.doc(fb.db,"contributions",user.email), {
      ...
      .then(()=> showToast("Recorded " + money(amt) + " \u2726"))
      .catch(e=>{ showToast("Couldn't update bar: " + (e.code||e.message)); console.error(e); });
  } else {
    data[user.email] = (data[user.email]||0) + amt; repaint();
    showToast("Recorded " + money(amt) + " \u2726 (preview)");
  }
}
function payNow(amt){
  if(amt<1){ return; }
...
$("#doPay").onclick = ()=>{
  ...
  if(amt<100){ showToast("Minimum is \u20b9100"); $("#inAmt").focus(); return; }
  if(!user){ closeM(payOverlay); openM(loginOverlay); return; }
  closeM(payOverlay);
  saveContribution(amt, "self-reported");
};
